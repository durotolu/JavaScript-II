...
let fullNames = runners.forEach(runners => `${runners.first_name} ${runners.last_name}`);
console.log(fullNames);

// ==== Challenge 2: Use .map() ====
// The event director needs to have all the runners' first names in uppercase because the director BECAME DRUNK WITH POWER. Populate an array called `firstNamesAllCaps`. This array will contain just strings.
let firstNamesAllCaps = runners.map(runners => `${runners.first_name.toUpperCase()} ${runners.last_name}`);
...
let ticketPriceTotal = runners.reduce((acc, runners) => {return acc + runners.donation}, 0);
